...
def test_scan_with_photo_round(tmp_path):
    ...
    assert mounts["usb0"]["pitch_mount_deg"] == 48.54
    assert mounts["usb0"]["az_offset_deg"] == 19.8
...
def test_load_mounts_defaults_and_override(tmp_path):
    ...
    assert mounts["usb1"].az_offset_deg == 212.0
    ...
    assert mounts["usb0"].az_offset_deg == 19.8      # Default bleibt
