...
```bash
./venv/bin/scanorama selftest                      # LiDAR delivering data?
./venv/bin/scanorama motor-test --degrees 5 --back # motor turns + direction ok?
...
Then `scripts/deploy.sh` rsyncs the working tree to `~/scanorama` on the
